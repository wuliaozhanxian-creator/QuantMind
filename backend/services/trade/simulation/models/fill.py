...
class SimulationFill(Base, TimestampMixin):
    ...
    legacy_trade_id: Mapped[int | None] = mapped_column(
    ...
    session_phase: Mapped[str | None] = mapped_column(String(32), nullable=True)

    __table_args__ = (
        Index("idx_simulation_fills_owner_symbol", "tenant_id", "user_id", "symbol"),
        Index(
            "idx_simulation_fills_owner_executed", "tenant_id", "user_id", "executed_at"
        ),
    )
