...
- `SimulationAccountManager.init_account/update_balance` 现会同步写入 `initial_equity/day_open_equity/month_open_equity` 与 `baseline.*`，避免首次成交后账户现金已变动但初始权益仍缺失，导致仪表盘把全部持仓误判为累计盈利。
...
- `/api/v1/simulation/account` 在 `sim_trades` 尚未形成可回放成交、只能回退 Redis 持仓快照时，会优先读取 `account.initial_equity / baseline.initial_equity`；若历史缓存缺失这些字段，则退化按 `cash + 持仓成本` 重建初始权益，避免把整笔持仓市值直接算成总盈亏。
...
- 若上述原生策略启动时恰好落在 hosted scheduler 当日触发窗口内，bootstrap 任务会复用调度器的 `task_id + Redis lock` 去重，避免启动瞬间与定时调度同时下单，前端因此看到两笔买入被重复计入。
