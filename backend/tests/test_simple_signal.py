...
def test_simple_signal_loads_first_column_from_qlib_instrument_file(tmp_path: Path):
    ...
    assert signal._load_instruments_from_file(instrument_file) == [
        "SH600000",
        "SZ000001",
    ]
...
def test_simple_signal_lags_pred_series_to_next_available_date(tmp_path: Path):
    ...
    series = signal._load_pred_series(
        pd.Timestamp("2025-01-03"), pd.Timestamp("2025-01-03")
    )
...
def test_simple_signal_with_pred_path_returns_empty_when_slice_missing(
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            AssertionError("pred-backed signal should not fallback to feature loading")
        ),
    )
